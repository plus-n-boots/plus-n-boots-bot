...
export default class {
  ...
  setupData (repoId, issueId) {
    this.data[repoId] = {}
    this.data[repoId][issueId] = {}
    this.data[repoId][issueId].count = 0
    this.data[repoId][issueId].users = []
    return this.data
  }
  /**
  ...
  async commentIssue (event) {
    this.data = this.setupData(event.repository.id, event.issue.id)
    let issue = this.data[event.repository.id][event.issue.id]
